...
def refresh_torrents():
    ...
    try:
        ...
        for raw_torrent in raw_torrents_from_prowlarr:
            ...
            if not torrent:
                current_app.logger.debug(f"Skipping torrent because it failed normalization: {raw_torrent.get('title')}")
                continue
            if torrent['hash'] in ignored_hashes:
                current_app.logger.debug(f"Skipping torrent because it is in the ignore list: {torrent['title']}")
                continue
            ...
            if torrent['hash'] not in existing_torrents_map:
                ...
                for kw in exclude_keywords:
                    if re.search(kw, torrent['title'], re.IGNORECASE):
                        current_app.logger.debug(f"Skipping torrent due to excluded keyword '{kw}': {torrent['title']}")
                ...
                if media_type == 'movie':
                    ...
                    if year_match and int(year_match.group(1)) < min_movie_year:
                        current_app.logger.debug(f"Skipping movie torrent due to year filter (< {min_movie_year}): {torrent['title']}")
                        continue
                ...
                current_app.logger.debug(f"Skipping torrent because it already exists: {torrent['title']}")

        current_app.logger.info(f"Added {new_torrents_added} new unique torrents to the processing list.")
