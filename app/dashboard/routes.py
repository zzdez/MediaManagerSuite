...
def _normalize_torrent(raw_torrent):
    ...
    """
    # A torrent is invalid if it has neither a hash nor an ID.
    unique_id = raw_torrent.get('hash') or raw_torrent.get('id')
    if not unique_id:
        current_app.logger.warning(f"Skipping torrent with no hash or ID: {raw_torrent.get('title')}")
        return None
