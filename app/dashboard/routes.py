...
def get_last_refresh_times():
    ...
    try:
        with open(DASHBOARD_STATE_FILE, 'r') as f:
            ...
            prowlarr_ts = data.get('last_refresh_utc')
            if prowlarr_ts:
                default_times['prowlarr'] = datetime.fromisoformat(prowlarr_ts).replace(tzinfo=timezone.utc)

            status_ts = data.get('last_status_refresh_utc')
            if status_ts:
                default_times['status'] = datetime.fromisoformat(status_ts).replace(tzinfo=timezone.utc)
...
def refresh_torrents():
    ...
    try:
        ...
        refresh_times = get_last_refresh_times()
        last_refresh = refresh_times['prowlarr']
        prowlarr_categories = get_dashboard_categories()
