...
def refresh_torrents():
    ...
    try:
        ...
        current_app.logger.info(f"--- Starting Dashboard Filtering ---")
        current_app.logger.info(f"Last Refresh Time: {last_refresh_utc}")
        current_app.logger.info(f"Exclude Keywords: {exclude_keywords}")
        current_app.logger.info(f"Min Movie Year: {min_movie_year}")
        ...
        for raw_torrent in raw_torrents:
            # Step 1: Normalize the raw data from Prowlarr
            torrent = _normalize_torrent(raw_torrent)

            # Step 2: Filter based on normalized data
            if not torrent['hash']:
                current_app.logger.info(f"FILTER (No Hash): '{torrent['title']}'")
                filtered_count += 1
                continue
            if torrent['hash'] in ignored_hashes:
                current_app.logger.info(f"FILTER (Ignored Hash): '{torrent['title']}'")
                filtered_count += 1
                continue
            ...
            if torrent['publishDate'] and last_refresh_utc:
                if torrent['publishDate'] <= last_refresh_utc:
                    current_app.logger.info(f"FILTER (Date): '{torrent['title']}' (Published: {torrent['publishDate']})")
                    filtered_count += 1
                    continue
            ...
            if keyword_match:
                current_app.logger.info(f"FILTER (Keyword: '{keyword_match}'): '{torrent['title']}'")
                filtered_count += 1
                continue
            ...
            if torrent['type'] == 'movie':
                ...
                if year_match and int(year_match.group(1)) < min_movie_year:
                    current_app.logger.info(f"FILTER (Year < {min_movie_year}): '{torrent['title']}'")
                    filtered_count += 1
                    continue
        ...
        current_app.logger.info(f"Total items processed: {len(raw_torrents)}")
        current_app.logger.info(f"Items filtered out: {filtered_count}")
        current_app.logger.info(f"Items returned to frontend: {len(final_torrents)}")
        current_app.logger.info(f"--------------------------")
...
def _normalize_torrent(raw_torrent):
    ...
    for the MMS frontend and internal logic.
    """
    ...
    if publish_date_str:
        try:
            ...
            current_app.logger.warning(f"Could not parse date: {publish_date_str}")

    return {
        'hash': raw_torrent.get('hash'),
