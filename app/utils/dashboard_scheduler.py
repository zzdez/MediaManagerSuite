...
LOCK_FILE = os.path.join('instance', 'dashboard_refresh.lock')
...
def scheduled_dashboard_refresh():
    ...
    if os.path.exists(LOCK_FILE):
        current_app.logger.warning("Scheduler: Refresh job already in progress. Skipping execution.")
        return None
    ...
    try:
        ...
        if os.path.exists(LOCK_FILE):
            os.remove(LOCK_FILE)
            current_app.logger.info("Scheduler: Refresh lock released.")
