...
def get_user_specific_plex_server(silent=False):
    ...
    Handles impersonation for managed users. Returns None on failure.
    If silent=True, no flash messages will be shown on failure and it will fallback to admin server.
    """
    current_app.logger.debug("--- Appel de get_user_specific_plex_server ---")
    if 'plex_user_id' not in session:
        if silent:
            current_app.logger.info("Aucun utilisateur en session, utilisation du serveur admin en mode silencieux.")
            return get_plex_admin_server()
