        ...
        if min_date:
            try:
                ...
                if page_dates and all(d < min_date for d in page_dates):
                    ...
                    # if Prowlarr returned cached/stale data.
                    # We only stop if:
                    # 1. We are NOT on the first page (offset > 0)
                    # 2. OR We are on the first page, but we want to be strict.
                    # Given the user issue, let's relax: If offset=0, we only WARN but CONTINUE to next page
                    # to see if newer items are hiding in page 2 (bad sort?).
                    # Actually, if page 1 is ALL old, page 2 should be even older (if sorted).
                    # But if cache is weird, maybe not?
                    # Let's keep it simple: Just stop.
                    # BUT add a log if the very first item is older than min_date.

                    if offset == 0 and page_dates[0] < min_date:
                         logging.warning(f"  [Time Gap Detected] The newest item from Prowlarr ({page_dates[0]}) is OLDER than your last local refresh ({min_date}). This implies Prowlarr returned stale data or no new items exist. Stopping pagination.")
