...
def get_latest_from_prowlarr(categories, min_date=None):
    ...
    while page <= max_pages:
        # Use strict offset-based pagination.
        # Evidence shows 'page' parameter is ignored by Prowlarr for this search, causing infinite loops.
        # 'offset' + 'limit' is supported by the internal API and maps to the indexer queries.
        offset = (page - 1) * pageSize
        limit = pageSize

        logging.info(f"Requesting Prowlarr Page: {page}/{max_pages} (Offset: {offset})...")

        # Do NOT send 'page' or 'pageSize' to avoid conflicts or defaults.
