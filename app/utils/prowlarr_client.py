...
def get_latest_from_prowlarr(categories, min_date=None):
    ...
    while page <= max_pages:
        ...
        # Some Prowlarr/Indexer implementations ignore 'page' or behave inconsistently with it.
        # 'offset' + 'limit' is the most deterministic method for the underlying Torznab APIs.
        offset = (page - 1) * pageSize
        limit = pageSize

        logging.info(f"Requesting Prowlarr Page: {page}/{max_pages} (Offset: {offset})...")

        # We purposely do NOT send 'page' to avoid conflict or double-offsetting.
