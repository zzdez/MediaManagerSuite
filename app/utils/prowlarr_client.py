...
def get_latest_from_prowlarr(categories, min_date=None):
    ...
    while page <= max_pages:
        # Calculate offset for indexers that require it
        offset = (page - 1) * pageSize
        limit = pageSize

        logging.info(f"Requesting Prowlarr Page: {page}/{max_pages} (Offset: {offset})...")

        # Pass page/pageSize AND offset/limit to cover different API expectations
        # Crucial: restore sort order to ensure pagination moves backwards in time
