...
# --- Route select_user ---
# (Votre code - Inchangé)
@plex_editor_bp.route('/select_user', methods=['GET', 'POST'])
def select_user():
    users_list = []
    ...
    if request.method == 'POST':
        ...
        if user_id_selected and user_title_selected:
            ...
            current_app.logger.warning("select_user: Soumission du formulaire avec données manquantes.")
    try:
        ...
        current_app.logger.debug(f"select_user: Liste des utilisateurs pour sélection: {len(users_list)}.")
    except Exception as e:
        current_app.logger.error(f"select_user: Erreur lors de la construction de la liste des utilisateurs: {e}", exc_info=True)
