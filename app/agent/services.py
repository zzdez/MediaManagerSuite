...
def generate_youtube_queries(title, year, media_type):
    ...
    model_name = current_app.config.get('GEMINI_MODEL_NAME') # Lis la config
    ...
    try:
        ...
        model = genai.GenerativeModel(model_name) # Utilise la variable
        ...
        print(f"ERREUR lors de la génération des requêtes Gemini avec le modèle '{model_name}': {e}")
