...
def create_app(config_class=Config):
    ...
    def format_iso_datetime(iso_string, show_time=True):
        """Filtre Jinja pour formater une date/heure ISO en format lisible."""
        if not iso_string:
            return 'Date inconnue'
        try:
            # Utilisation de strptime pour une compatibilité étendue
            # Gère les 'Z' et les microsecondes
            if isinstance(iso_string, datetime.datetime):
                dt = iso_string
            else:
                core_string = iso_string.split('.')[0].rstrip('Z')

                # Essayer de parser avec l'heure, puis sans
                try:
                    dt = datetime.datetime.strptime(core_string, '%Y-%m-%dT%H:%M:%S')
                except ValueError:
                    dt = datetime.datetime.strptime(core_string, '%Y-%m-%d')
