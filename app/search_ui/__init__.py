...
import logging
from flask import Blueprint, render_template, request, flash, jsonify, Response, stream_with_context, current_app, url_for, session, redirect
...
@search_ui_bp.route('/api/get_search_queries', methods=['GET'])
@login_required
def get_search_queries():
    """Fournit et supprime les requêtes de recherche en attente de la session."""
    queries = session.pop('missing_episodes_queries', None)
    search_mode = session.pop('search_mode_intent', None)
    return jsonify({'queries': queries, 'search_mode': search_mode})
